                    ...
                    html.Div(className='offcanvas-body', children=[
                        html.Ul(className='navbar-nav justify-content-end flex-grow-1 pe-3', children=[
                            html.Li(className='nav-item', children=[
                                dcc.Link('Home', className='nav-link active', href='/'),
                            ]),
                            html.Li(className='nav-item', children=[
                                dcc.Link('Upload and view transactions', className='nav-link', href='/templates/transactions', target = "_blank"),
                                dcc.Link('Home', className='nav-link', href='/templates/'),
                            ]),
                        ]),
                    ]),
                    ...
                    html.Div(className='col-3 mx-auto my-5', children=[
                        html.Div(className='card', style={'borderColor': 'white'}, children=[
                            html.Div(className='card-body', style={'backgroundColor': 'white', 'textAlign': 'center'}, children=[
                                dcc.Link('Back to view transactions', className='btn btn-secondary', style={'color': 'white'}, href='/templates/transactions', target = "_blank"),
                            ]),
                        ]),
                    ]),
                ]),
...
def open_local_file():
    ...
    return dcc.Location(pathname='/templates/index.html')

@dash_app.server.route('/transactions')
def open_local_file():
    subprocess.call(["python", "-m", "http.server", "8000"])
    return dcc.Location(pathname='/templates/transactions.html')
